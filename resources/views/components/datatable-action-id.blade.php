        ...
        <table class="table table-bordered table-striped" id="{{$tableId}}">
            ...
            @foreach($data as $key => $value)
                <tr>
                    ...
                    @endforeach
                    <td>
                        <a href="{{route($deleteRoute['route'], ['Rec_Cnt' => data_get($value,$deleteRoute['key'])])}} " class="btn rounded-pill btn-outline-danger btn-xs" data-confirm-delete="true">
                            <i class="bx bx-trash me-1"></i>
                            {{trans('buttoncontrol.delete_btn',[],session()->get('locale'))}}
                        </a>
                        <a href="{{route($editRoute['route'],['action'=>'edit','Rec_Cnt'=>data_get($value, $editRoute['key'])]) }}" class="btn rounded-pill btn-outline-warning btn-xs">
