...
@section('content')
    ...
    <div class="col col-12 flex-column">
        <h4 class="fw-bold py-3 mb-4">
            <span class="text-muted fw-light">ລົງບັນຊີຈ່າຍລ່ວງໜ້າ</span>
            <a href="{{ route('VoucherAdvance.addva', ['action' => 'more', 'key' => null]) }}"
    ...
    <div class="card p-3">
        <form method="GET" action="{{ route('VoucherAdvance.index') }}">
                    ...
                    @foreach ($vouchers as $key => $value)
                        <tr>
                            <td class="text-nowrap">
                                <a href="{{ route('VoucherAdvance.update', ['key' => $value->Vch_AutoNo]) }}"
                                ...
                                <a href="{{ route('print.showVoucherGeneral', ['level' => $value->LevelID, 'implementCD' => $value->ImplementCD, 'voucherNo' => $value->Vch_AutoNo, 'type' => 'advance', 'path' => 'VoucherAdvance']) }}"
        ...
        <script>
            ...
            $(document).on('click', '#btn-del', function(e) {
                ...
                Swal.fire({
                    ...
                    if (rs.isConfirmed) {
                        axios.get(`${url}/VoucherAdvance/destroy?keyId=${keyId}`).then(() => {
                            Swal.fire({
                                toast: true,
                                position: 'top-right',
                                title: 'ສຳເລັດ',
