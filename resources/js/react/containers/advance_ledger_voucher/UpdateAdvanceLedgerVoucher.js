...
const UpdateAdvanceLedgerVoucher = () => {
    // get id from url
    ...
    const fetchGetSingleVoucher = async (p_voucher_id) => {
        try {
            const response = await axios.get(
                `/VoucherAdvance/add/data/${p_voucher_id}`
            );
            const data = response.data;
            if (data) {
                setinputAdvanceVoucherMain({
...
const root = ReactDOM.createRoot(
    document.getElementById("update-advance-ledger-voucher")
);

root.render(<UpdateAdvanceLedgerVoucher />);
