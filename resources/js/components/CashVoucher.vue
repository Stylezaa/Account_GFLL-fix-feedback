...
export default {
    ...
    methods: {
        async loadSingleData() {
            try {
                const query = new URLSearchParams(window.location.search);
                await axios.get(`${process.env.MIX_APP_URL}:${process.env.MIX_APP_PORT}/CashVoucher/add/data/${query.get('key')}`)
                    .then((res) => {
        ...
        async loadingAccounts() {
            try {
                const res = await axios.get(`${process.env.MIX_APP_URL}:${process.env.MIX_APP_PORT}/CashVoucher/accounts`);
                if (res.data) {
        ...
        async loadProvince() {
            try {
                const res = await axios.get(`${process.env.MIX_APP_URL}:${process.env.MIX_APP_PORT}/CashVoucher/provinces`);
                if (res.data !== null) {
        ...
        async loadDistrict() {
            try {
                ...
                if (provinceId !== null) {
                    res = await axios.get(`${process.env.MIX_APP_URL}:${process.env.MIX_APP_PORT}/CashVoucher/districts?provinceId=${provinceId}`);
                } else {
                    res = await axios.get(`${process.env.MIX_APP_URL}:${process.env.MIX_APP_PORT}/CashVoucher/districts`);
                }
                if (res.data !== null) {
        ...
        async loadVillage() {
            try {
                ...
                if (this.store.district !== null) {
                    ...
                    res = await axios.get(`${process.env.MIX_APP_URL}:${process.env.MIX_APP_PORT}/CashVoucher/villages?districtId=${districtId}`);
                } else {
                    res = await axios.get(`${process.env.MIX_APP_URL}:${process.env.MIX_APP_PORT}/CashVoucher/villages`);
                }
                if (res.data !== null) {
        ...
        async loadLevels() {
            try {
                const res = await axios.get(`${process.env.MIX_APP_URL}:${process.env.MIX_APP_PORT}/CashVoucher/levels`);
                if (res.data !== null) {
                ...
                if (valid) {
                    ...
                    if (checking) {
                        ...
                        if (this.accountParing.length > 0 && this.convertToNum(this.store.lakBalance) === 0 && this.convertToNum(this.store.usdBalance) === 0) {
                            ...
                            if (query.get('action') === 'edit') {
                                await axios.put(`${process.env.MIX_APP_URL}:${process.env.MIX_APP_PORT}/CashVoucher/update?keyId=${query.get('key')}`, {
                                    voucher: this.store,
                                ...
                                await axios.post(`${process.env.MIX_APP_URL}:${process.env.MIX_APP_PORT}/CashVoucher/store`, {
                                    voucher: this.store,
                                    detail: this.accountParing
                                }).then((res) => {
                                    if (res.status === 200 || res.status === 201) {
        ...
        resetFormVal() {
            const query = new URLSearchParams(window.location.search);
            if (query.get('action') === 'edit') {
                window.location.href = `${process.env.MIX_APP_URL}:${process.env.MIX_APP_PORT}/CashVoucher/add?action=more`;
            }
        ...
        preview() {
            window.open(`${process.env.MIX_APP_URL}:${process.env.MIX_APP_PORT}/CashVoucher/preview?level=${this.store.level}&implementCD=${this.implementCode()}&voucherNo=${this.fallbackData.Vch_AutoNo}`, '_blank')
        },
