...
export default {
    ...
    methods: {
        preview(item, action) {
            if (action === 'edit') {
                window.open(`${process.env.MIX_APP_URL}:${process.env.MIX_APP_PORT}/BankVoucher/add?action=edit&key=${item.Vch_AutoNo}`, '_blank')
            } else {
                window.open(`${process.env.MIX_APP_URL}:${process.env.MIX_APP_PORT}/BankVoucher/preview/print?level=${item.LevelID}&implementCD=${item.ImplementCD}&voucherNo=${item.Vch_AutoNo}`, '_blank')
            }
        },
        deleteAction(item) {
            axios.get(`${process.env.MIX_APP_URL}:${process.env.MIX_APP_PORT}/BankVoucher/destroy?keyId=${item.Vch_AutoNo}`).then(() => {
        ...
        async loadSingleItem(item) {
            try {
                const res = await axios.get(`${process.env.MIX_APP_URL}:${process.env.MIX_APP_PORT}/BankVoucher/details/preview/data?voucherId=${item.Vch_AutoNo}`)
