...
export default {
    ...
    methods: {
        async loadSingleData() {
            try {
                const query = new URLSearchParams(window.location.search);
                await axios.get(`${process.env.MIX_APP_URL}:${process.env.MIX_APP_PORT}/BankVoucher/add/data/${query.get('key')}`)
                    .then((res) => {
                ...
                if (valid) {
                    ...
                    if (checking) {
                        ...
                        if (this.accountParing.length > 0 && this.convertToNum(this.store.lakBalance) === 0 && this.convertToNum(this.store.usdBalance) === 0) {
                            ...
                            if (query.get('action') === 'edit') {
                                await axios.put(`${process.env.MIX_APP_URL}:${process.env.MIX_APP_PORT}/BankVoucher/update?keyId=${query.get('key')}`, {
                                    voucher: this.store,
                                ...
                                await axios.post(`${process.env.MIX_APP_URL}:${process.env.MIX_APP_PORT}/BankVoucher/store`, {
                                    voucher: this.store,
                                    detail: this.accountParing
                                }).then((res) => {
                                    if (res.status === 200 || res.status === 201) {
        ...
        resetFormVal() {
            const query = new URLSearchParams(window.location.search);
            if (query.get('action') === 'edit') {
                window.location.href = `${process.env.MIX_APP_URL}:${process.env.MIX_APP_PORT}/BankVoucher/add?action=more`;
            }
        ...
        preview() {
            window.open(`${process.env.MIX_APP_URL}:${process.env.MIX_APP_PORT}/BankVoucher/preview?level=${this.store.level}&implementCD=${this.implementCode()}&voucherNo=${this.fallbackData.Vch_AutoNo}`, '_blank')
        },
