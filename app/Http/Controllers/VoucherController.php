...
class VoucherController extends Controller
{
    public function index(Request $request)
    {
        ...
        $vouchers = Voucher::with(['province', 'district', 'village', 'items'])
            ->where('VoucherType', 'VJN')
            ->whereBetween('created_at', [$startDate . " 00:00:00", $presentDate . " 23:59:59"])
            ->orderBy('Rec_Cnt', 'DESC')
            ->get();
        //dd($vouchers);
        return view('voucher.index', compact('vouchers', 'startDate', 'presentDate'));
    }
    ...
    public function previewData(Request $request)
    {
        ...
        $voucher = DB::select("SELECT * FROM dbo.RPTVoucher('$request->level','$request->implementCD','$request->voucherNo','VCHGJN', '$lang')");
    ...
    public function getLastVoucherNo($level): string
    {
        ...
        if ($voucher){
            ...
            $year = date('y');
            return "VJN".$level.substr($year,0,2)."00000";
        }
    }
