...
class VoucherAdvanceController extends Controller
{
    public function index(Request $request)
    {
        ...
        $vouchers = Voucher::with(['province', 'district', 'village', 'items'])
            ->where('VoucherType', 'VAD')
            ->whereBetween('created_at', [$startDate . " 00:00:00", $presentDate . " 23:59:59"])
            ->orderBy('Rec_Cnt', 'DESC')
            ->get();
        return view('voucherAdvance.index', compact('vouchers', 'startDate', 'presentDate'));
    }

    public function addva()
    {
        return view('voucherAdvance.addva');
    }
    ...
    {
        $vouchers = Voucher::with(['province', 'district', 'village', 'items', 'items.activity', 'items.activity.category', 'items.donor', 'items.account', 'items.costCenter', 'items.subCostCenter'])
    ...
    {
        return view('voucherAdvance.preview');
    }
    ...
    public function previewData(Request $request)
    {
        ...
        $voucher = DB::select("SELECT * FROM dbo.RPTVoucher('$request->level','$request->implementCD','$request->voucherNo','VCHADV', '$lang')");
    ...
    public function storeVoucherItem($request, Voucher $voucher, $itemCount): void
    {
        ...
        $voucherItem->ActivityID = $request['activity']['ActivityID'];
        ...
        $voucherItem->USD_Dr = $request['amountDebitUsd'];
        $voucherItem->USD_Cr = $request['amountCreditUsd'];
        $voucherItem->iRate = intval(str_replace(',', '', $request['rate']));
        $voucherItem->LAK_Dr = $request['amountDebitLak'];
        $voucherItem->LAK_Cr = $request['amountCreditLak'];
    ...
    public function storeVoucherData($request): Voucher
    {
        ...
        $voucher->Voucher_Amt = intval(str_replace(',', '', $request['voucher']['amount']));
        ...
        $voucher->VoucherType = "VAD";
        $voucher->Amt_USD_Dr = array_sum(array_column($request['detail'], 'amountDebitUsd'));
        $voucher->Amt_USD_Cr = array_sum(array_column($request['detail'], 'amountCreditUsd'));
        $voucher->Amt_LAK_Dr = array_sum(array_column($request['detail'], 'amountDebitLak'));
        $voucher->Amt_LAK_Cr = array_sum(array_column($request['detail'], 'amountCreditLak'));
    ...
    public function updateVoucher($request, $key)
    {
        Voucher::where('Vch_AutoNo', $key)->update([
            ...
            'Voucher_Amt' => intval(str_replace(',', '', $request['voucher']['amount'])),
            ...
            'VoucherType' => "VAD",
            'Amt_USD_Dr' => array_sum(array_column($request['detail'], 'amountDebitUsd')),
            'Amt_USD_Cr' => array_sum(array_column($request['detail'], 'amountCreditUsd')),
    ...
    public function getLastVoucherNo($level): string
    {
        ...
        if ($voucher) {
            ...
            $year = date('y');
            return "VAD" . $level . substr($year, 0, 2) . "00000";
        }
    }

    public function getAdvanceVoucherJson(Request $request)
    {
        $startDate = $request->startDate ? $request->startDate : Carbon::now()->firstOfMonth()->format('Y-m-d');
        $presentDate = $request->presentDate ? $request->presentDate : Carbon::now()->format('Y-m-d');
        ...
        $vouchers = Voucher::with(['province', 'district', 'village', 'items'])
            ...
            ->when($voucherNo, function ($query) use ($voucherNo) {
                return $query->where('Vch_AutoNo', $voucherNo);
            })
            ->where('VoucherType', 'VAD')
            ->where('Clear_Adv', 0)
            ->whereBetween('created_at', [$startDate . " 00:00:00", $presentDate . " 23:59:59"])
