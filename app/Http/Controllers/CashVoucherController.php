...
class CashVoucherController extends Controller
{
    public function index(Request $request)
    {
        ...
        $vouchers = Voucher::with(['province', 'district', 'village', 'items'])
            ->where('VoucherType', 'VCH')
            ->whereBetween('created_at', [$startDate . " 00:00:00", $presentDate . " 23:59:59"])
            ->orderBy('Rec_Cnt', 'DESC')
            ->get();//return json_encode($vouchers);

       return view('cashVoucher.index', compact('vouchers', 'startDate', 'presentDate'));
    }

    public function add()
    {
        return view('cashVoucher.add');
    }
    ...
    {
        return view('cashVoucher.preview');
    }

    public function print()
    {
        return view('cashVoucher.print');
    }
    ...
    public function destroy(Request $request)
    {
        ...
        return redirect()->route('cashVoucher.index');
    }
    ...
    public function getLastVoucherNo($level): string
    {
        ...
        if ($voucher) {
            ...
            $year = date('y');
            return "VCH" . $level . substr($year, 0, 2) . "00000";
        }
    }
